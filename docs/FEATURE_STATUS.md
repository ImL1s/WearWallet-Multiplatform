...
| --- | --- | --- | --- | --- | --- |
| `wear_send` | Wear OS send | `BETA` | Yes | Signing possible only if a capability gate allows it | Address checksum enforcement and fee-fallback policy are **Task D** leftovers. Treat send as incomplete. A returned tx hash is **not** on-chain confirmation. |
