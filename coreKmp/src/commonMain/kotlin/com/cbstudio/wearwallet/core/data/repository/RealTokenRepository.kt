...
class TokenRepositoryImpl(
    ...
    override suspend fun scanUserTokens(
        ...
        mergePersistedUserTokens(tokens, walletAddress, chainType)
    ...
    override suspend fun saveUserToken(walletAddress: String, token: Token) {
        val walletId = requireWalletId(walletAddress)
        database.tokenQueries.insert(
            ...
            chain_type = token.chainType.name,
            chain_id = token.chainType.getChainId(),
            balance = token.balance,
            usd_price = token.usdPrice,
            price_change_24h = null,
            logo_url = token.logoUrl,
            is_native = if (token.isNative) 1L else 0L,
            is_hidden = 0L,
            contract_type = "ERC20",
            metadata = "{}"
        )
    ...
    override suspend fun removeUserToken(walletAddress: String, tokenAddress: String) {
        val walletId = requireWalletId(walletAddress)
        database.tokenQueries.deleteByWalletIdAndAddress(walletId, tokenAddress)
    }
    ...
    private fun requireWalletId(walletAddress: String): Long {
        return database.walletQueries.selectByAddress(walletAddress).executeAsOneOrNull()?.id
            ?: throw IllegalStateException("Cannot persist custom token: wallet row not found")
    }

    private fun mergePersistedUserTokens(
        tokens: MutableList<Token>,
        walletAddress: String,
        chainType: ChainType
    ) {
        val wallet = database.walletQueries.selectByAddress(walletAddress).executeAsOneOrNull() ?: return
        val persisted = database.tokenQueries.selectByWalletId(wallet.id).executeAsList()
            .filter { row ->
                row.chain_type == chainType.name || row.chain_id == chainType.getChainId()
            }
            .map { row -> row.toDomainToken() }
        persisted.forEach { persistedToken ->
            if (tokens.none { it.address.equals(persistedToken.address, ignoreCase = true) }) {
                tokens.add(persistedToken)
            }
        }
    }

    private fun com.cbstudio.wearwallet.core.database.Token.toDomainToken(): Token {
        return Token(
            id = id.toString(),
            address = address,
            name = name,
