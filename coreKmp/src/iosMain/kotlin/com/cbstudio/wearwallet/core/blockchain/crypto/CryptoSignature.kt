 ...
 *
 * 🔧 遷移記錄：
 * - ❌ 2025-10-10: TrustWallet Core 不支援 watchOS
 * - ✅ 2025-10-10: 改用 curve25519-kotlin 純 Kotlin 實現 Ed25519
 * - ✅ 2025-10-20: 遷移到 Secp256k1Pure，移除 libsecp256k1 C 庫依賴
 */
actual object CryptoSignature {
    ...
    actual fun signWithEd25519(message: String, privateKeyHex: String): String {
        ...
        return try {
            ...
            SecureCryptoLogger.success("iOS Ed25519 真實簽名成功")
            signatureHex
    ...
    actual fun signWithECDSA(message: String, privateKeyHex: String): String {
        ...
        return try {
            ...
            SecureCryptoLogger.success("iOS ECDSA 簽名成功 (Secp256k1Pure)")
            signatureHex
    ...
    actual fun signDigest(digest: ByteArray, privateKeyHex: String): String {
        val privateKeyBytes = hexToByteArray(privateKeyHex)
        return try {
            val signature = Secp256k1Pure.sign(digest, privateKeyBytes)
     ...
     *
     * ✅ Ed25519 真實驗證 (curve25519-kotlin)
     * ✅ ECDSA secp256k1 真實驗證 (Secp256k1Pure)
     *
    ...
    actual fun derivePublicKeyFromPrivateKey(privateKeyHex: String): String {
        ...
        return try {
            ...
            SecureCryptoLogger.success("iOS 公鑰派生成功 (Secp256k1Pure)")
            publicKeyHex
    ...
    actual fun recoverPublicKey(
        ...
        return try {
            val hashBytes = hexToByteArray(messageHash)
            val rBytes = hexToByteArray(r)
            val sBytes = hexToByteArray(s)
            val point = io.github.iml1s.crypto.Secp256k1Pure.recoverPublicKeyPoint(hashBytes, rBytes, sBytes, recoveryId) ?: return null
            val uncompressed = io.github.iml1s.crypto.Secp256k1Pure.pointToByteArray(point, compressed = false)
            bytesToHex(uncompressed)
        } catch (e: Exception) {
            SecureCryptoLogger.error("RECOVER_PUBKEY_FAILED", e.message ?: "Unknown error")
            null
        }
    }
