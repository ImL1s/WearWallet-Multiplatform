...
class CreateWalletViewModelTest : KoinTest {
    ...
    @Test
    fun `SHOW_MNEMONIC does not auto-complete after 60 seconds without confirmBackup`() = runTest {
        viewModel.acknowledgeWarning()
        assertEquals(CreateWalletViewModel.CreationStep.SHOW_MNEMONIC, viewModel.uiState.value.currentStep)
        assertFalse(viewModel.uiState.value.walletCreated)
        ...
        assertEquals(CreateWalletViewModel.CreationStep.SHOW_MNEMONIC, state.currentStep)
        assertTrue(state.showMnemonic)
        assertFalse(state.walletCreated)
    }
}
