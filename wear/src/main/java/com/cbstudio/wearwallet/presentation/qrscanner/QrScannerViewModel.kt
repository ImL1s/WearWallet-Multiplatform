...
class QrScannerViewModel(application: Application) : AndroidViewModel(application) {
    ...
    /**
     * Debug/emulator helper: inject a scanned payload without a paired phone camera.
     * Refuses the payload unless the QA harness is active. Not a production scan path.
     */
    fun simulateScan(payload: String) {
        val accepted = WearQaFixtures.acceptSimulatedQrScan(payload, WearQaHarness.isActive())
            ?: return
        _uiState.update {
            it.copy(
                scanResult = accepted,
