        ...
        viewModelScope.launch {
            try {
                ...
                val merged = WearQaFixtures.mergeHistoryPage(
                    existing = _uiState.value.transactions,
                    networkPage = transactions,
                    refresh = refresh,
                    harnessActive = WearQaHarness.isActive()
                )
                _uiState.update { 
                    it.copy(
                ...
                Timber.e(e, "載入交易記錄失敗")
                val fallback = WearQaFixtures.overlayTransactions(emptyList(), WearQaHarness.isActive())
                val networkError = "載入交易記錄失敗: ${e.message}"
                _uiState.update { 
                    it.copy(
