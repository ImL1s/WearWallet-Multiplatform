...
object TestTags {
    ...
    const val QR_SCAN_BUTTON = "qr_scan_button"
    const val QR_SCANNER_SCREEN = "qr_scanner_screen"
    const val QR_START_SCAN_BUTTON = "qr_start_scan_button"
    const val QR_SIMULATE_SCAN_BUTTON = "qr_simulate_scan_button"
    const val TOKEN_LIST_ADD_BUTTON = "token_list_add_button"
    const val TX_FILTER_ALL = "tx_filter_all"
    const val TX_FILTER_SENT = "tx_filter_sent"
    const val TX_FILTER_RECEIVED = "tx_filter_received"
    const val QA_FIXTURE_BANNER = "qa_fixture_banner"

    fun tokenRow(symbol: String) = "token_row_$symbol"
    fun txRow(hash: String) = "tx_row_${hash.takeLast(8)}"
    fun contactRow(id: String) = "contact_row_$id"
