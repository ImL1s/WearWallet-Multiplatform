            ...
            try {
                ...
                when (result) {
                    is Result.Success -> {
                        ...
                        Timber.d("載入聯絡人成功: ${contacts.size} 個")
                    }
                    is Result.Failure -> {
                        val fallback = WearQaFixtures.overlayContacts(
                            emptyList(),
                            chain,
                            WearQaHarness.isActive()
                        )
                        _uiState.update { 
                            it.copy(
                                contacts = fallback,
                                filteredContacts = fallback,
                                isLoading = false,
                                error = WearQaFixtures.retainedLoadError(
                                    networkError = "載入聯絡人失敗",
    ...
    fun onSearchQueryChange(query: String) {
        ...
        if (query.isEmpty()) {
            ...
            viewModelScope.launch {
                ...
                when (result) {
                    is Result.Success -> {
                        val chain = _uiState.value.currentChain
                        val chainFiltered = result.data.filter { it.chainType == chain }
                        val overlaid = WearQaFixtures.overlayContacts(
                            chainFiltered,
                            chain,
                            WearQaHarness.isActive()
                        )
                        val matching = overlaid.filter {
                            it.name.contains(query, ignoreCase = true) ||
                                it.address.contains(query, ignoreCase = true)
                        }
                        _uiState.update { 
                            it.copy(filteredContacts = matching)
                        }
                    }
                    is Result.Failure -> {
