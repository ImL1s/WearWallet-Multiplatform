...
import com.cbstudio.wearwallet.presentation.wallet.screens.settings.token.TokenManagementViewModel
import com.cbstudio.wearwallet.presentation.wallet.screens.main.tx.TransactionDetailViewModel
import com.cbstudio.wearwallet.presentation.ui.token.TokenTransactionHistoryViewModel
...
import com.cbstudio.wearwallet.core.security.AndroidBuildTypeProvider
import com.cbstudio.wearwallet.core.security.InMemorySecureKeyManager
...
val wearModule = module {
    ...
    viewModel { TokenTransactionHistoryViewModel() }
...
/** Debug-only DI overrides for emulator QA and screenshot capture (never active in release). */
private val debugWearOverridesModule = module {
    if (BuildConfig.DEBUG) {
        single<CapabilityGate> { AllowDevCapabilityGate(allowBroadcast = false) }
        single<com.cbstudio.wearwallet.core.security.BuildTypeProvider> {
            AndroidBuildTypeProvider(isDebug = true)
        }
        single<SecureKeyManager> {
            if (isEmulatorDevice()) {
                InMemorySecureKeyManager()
            } else {
                AndroidSecureKeyManager(androidContext())
            }
        }
    }
}
...
fun Context.initializeKoin() {
    ...
    try {
        startKoin {
            ...
            modules(wearModule + getAllWearModules() + androidNetworkModule + debugWearOverridesModule)
        }
